...
class RolePermissionAssignmentIntegrationTest extends AbstractIdmIntegrationTest {
    ...
    @Test
    void assign_and_unassign_permission_to_role_as_admin() throws Exception {
        ...
        roleReq.setName("R_ROLE_PERMISSION_TEST");
        roleReq.setDescription("role for role-permission assignment test");
        ...
        String roleBody = mockMvc.perform(post("/api/idm/roles")
                ...
                .andExpect(jsonPath("$.name", is("R_ROLE_PERMISSION_TEST")))
                .andReturn()
                .getResponse()
                .getContentAsString();
        ...
        group.setName("PG_ROLE_PERMISSION_TEST");
        group.setDescription("permission group for role-permission assignment test");
        ...
        permission.setName("P_ROLE_PERMISSION_TEST");
        permission.setDescription("permission for role-permission assignment test");
        ...
        assertThat(
                rolePermissionAssignmentRepository.findAllByRole_Id(roleId)
                        .stream()
                        .anyMatch(assignment -> assignment.getPermission().getId().equals(permissionId))
        ).isTrue();

        // unassign permission from role
        mockMvc.perform(delete("/api/idm/assignments/role-permission")
                        .header("Authorization", "Bearer " + token)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(request)))
                .andExpect(status().isOk());
